...
ENV INSTALL_PATH /sheedhet_server
# The name of the application is sheedhet_server and while there
...
# having to CD into the /sheedhet_server folder every time would be
# lame, so instead we can set the WORKDIR to be /sheedhet_server.
#
...
# sheedhet_server/ path inside of the Docker image.
#
# It copies it to /sheedhet_server because of the WORKDIR being set.
#
...
# /sheedhet_server folder inside of the Docker image.
#
...
RUN bundle exec rake RAILS_ENV=development DATABASE_URL=postgresql://user:pass@127.0.0.1/dbname ACTION_CABLE_ALLOWED_REQUEST_ORIGINS=foo,bar SECRET_TOKEN=dummytoken
